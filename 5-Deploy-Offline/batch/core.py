...
# Docker mounts Module 4's pipeline directly at /app/pipeline.
# Locally, the original repository structure is preserved.
if Path("/app/pipeline").exists():
    # ── Docker ────────────────────────────────────────────────────────────────
    _ONLINE_DIR = Path("/app")
...
#   src.features
#   old serialized model/preprocessor artifacts
...
# ── Data paths ────────────────────────────────────────────────────────────────
# BATCH_DATA_DIR overrides the local batch/ directory when running in Docker.

DATA_DIR = Path(
    os.getenv("BATCH_DATA_DIR", str(_CORE_DIR))
)
...
def load_champion() -> dict:
    ...
    and training baseline from MLflow.
    """
    ...
    if _ARTIFACTS_ROOT:
        ...
        model = mlflow.sklearn.load_model(
            _remap(
                _get_storage_location(mv.version)
            )
        )
    ...
    # ── Local / Prefect artifact handling ────────────────────────────────────
    ...
    train_mae = float(
        run.data.metrics["test_mae"]
    )

    train_mean = float(
        run.data.metrics["train_duration_mean"]
    )
...
def score_month(
    ...
    """
    Download ALL trips for one month and score every one.

    Returns:
        Aggregate metrics dict.

    Side effect:
        Writes predictions/YYYY_MM.parquet.
    """
    ...
    X = champion["preprocessor"].transform(
...
def get_all_results() -> list:
    """
    Read all rows from batch_results.db.
    """
...
def get_result(
    ...
    """
    Read one row from batch_results.db.
    """
