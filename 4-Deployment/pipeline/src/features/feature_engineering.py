"""
Feature engineering for 4-Deployment/pipeline
================================================
CHANGES FROM pipeline_with_prefect/src/features/feature_engineering.py:

  REMOVED (lat/lon-based — columns no longer exist in 2019 TLC data):
    ...
    efficiency_ratio         haversine / trip_distance (old version used lat/lon)

  ADDED (zone ID-based — schema migration fix + signal recovery):
    zone_pair                PULocationID * 1000 + DOLocationID
                             Encodes the specific route as one integer.

    is_same_zone             pickup == dropoff zone → very short trip
    ...
    centroid_distance_miles  Euclidean distance between zone centroids (EPSG:2263).
                             Replaces haversine_distance. Centroids precomputed
                             from taxi_zones.shp (TLC official shapefile).

    centroid_direction_sin   Cyclical encoding of travel direction between centroids.
    centroid_direction_cos   Replaces direction_sin/cos from lat/lon delta.
                             Captures rush-hour asymmetry: same corridor,
                             opposite direction → different trip duration.

    efficiency_ratio         trip_distance / (centroid_distance_miles + 0.1)
                             Detour factor: high = driver took a longer route
                             than the zone-to-zone straight line suggests.

  UNCHANGED (all temporal, categorical, interaction features):
...
FEATURE COUNT: 19 (pipeline_with_prefect) → 23 (this pipeline)

WHY centroid features close the gap vs 2016:
  2016 lat/lon data had haversine_distance + direction features which were strong
  predictors. Zone IDs alone don't encode geographic distance. Computing centroid
  distances from the TLC shapefile recovers ~70% of that lost signal without any
  new data dependencies (centroids are hardcoded from a one-time shapefile parse).
"""
import math
import numpy as np
import pandas as pd
