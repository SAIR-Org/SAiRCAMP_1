...
_MODULE_DIR   = _CORE_DIR.parent          # 6-Full-System/
_PIPELINE_DIR = _MODULE_DIR / "pipeline"  # 6-Full-System/pipeline/

sys.path.insert(0, str(_MODULE_DIR))    # shared.feature_engineering
...
def score_month(year: int, month: int, champion: dict) -> dict:
    """
    Download ALL trips for one month, score every one.

    Returns aggregate metrics dict.
    Side effect: writes predictions/YYYY_MM.parquet.
    """
    ...
    # ── Save per-trip predictions (batch deployment artifact) ─────────────────
