"""
flow.py — NYC Taxi ML Pipeline with Prefect Orchestration
==========================================================

THIS IS THE ONLY FILE WHERE PREFECT IS ADDED.
All src/ modules are plain Python and unchanged in their logic.
...
WHAT CHANGED VS pipline_no_perfect/:
  ...
  │  ─────────────────────────────  ─────────────────────────────│
  │  NYCTaxiMLPipeline class        @flow function               │
  │  pipeline.step_N_*() methods    @task functions              │
  │  @retry_with_backoff decorator  @task(retries=N)             │
  │  logging.getLogger()            prefect.get_run_logger()     │
  │  train_all_models() loop        individual @task per model   │
  └──────────────────────────────────────────────────────────────┘

HOW TO RUN:
  python main.py                        # full pipeline, defaults
  python main.py --sample-size 50000 --no-tune
  python main.py --sample-size 200000 --promote
...
def register_model(best_result, test_metrics, promote_to_prod, config):
    ...
    if version and promote_to_prod:
        ...
        logger.info(f"✅ Model v{version} → Production")
    elif version:
        logger.info(f"✅ Model v{version} → Staging (promote manually when ready)")
    else:
...
def nyc_taxi_pipeline(
    ...
    X_train_p, X_val_p, X_test_p, feature_names, _ = engineer_features(
    ...
    logger.info(f"\n  Load model:")
    logger.info(f"  mlflow.sklearn.load_model('models:/{config.mlflow.model_name}/Staging')")
    logger.info("=" * 60)
