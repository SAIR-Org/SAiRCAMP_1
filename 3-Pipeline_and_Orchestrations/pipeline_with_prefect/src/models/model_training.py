...
CHANGES FROM pipline_no_perfect/:
  ...
  ADDED:   build_model_portfolio(config) as a standalone module-level function.
  WHY:     The Prefect flow needs to iterate over models and submit each as a
           separate @task. A standalone function lets flow.py call it without
           instantiating ModelTrainer (which builds all 5 models on init).
...
Everything else is identical to pipline_no_perfect/src/models/model_training.py.
"""
import time
import numpy as np
import logging
